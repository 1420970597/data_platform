...
本报告针对**军事领域长链推理大模型**的数据制备平台，明确仅支持以下受控范围：

- 经组织批准、来源可追溯、许可或授权明确的非敏感资料；
- 公开教材、规范、历史档案、科研资料、装备维护的非敏感通用知识、文档理解和审阅辅助；
- 文本、扫描文档、图像、图表、表格和图文交错页面的归档、解析、检索、问答和训练数据治理；
- LoRA 领域适配与 GRPO 可验证奖励后训练的数据工程。

本平台**不支持**：实时态势、目标识别、目标选择、兵力部署、武器使用、作战行动、战术推演、指挥控制、未经授权情报收集或任何能够直接提升现实伤害能力的任务。数据处理的默认结果是“隔离、拒绝、转人工或转为边界说明”，而不是自动放行。
...
- 选型必须在已批准的非敏感黄金集上验证，而不是只看开源项目榜单；
