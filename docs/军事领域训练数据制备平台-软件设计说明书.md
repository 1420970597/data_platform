...
平台以“可追溯、可验证、可撤回、低人工成本”为核心，提供军事领域模型训练数据制备能力。设计目标是以可组合服务将战前准备、战时保障、战后复盘及联合演训中的原始授权资产转化为 LoRA 和 GRPO 两类可复现训练工件，并在每一环嵌入策略门禁和审计。数据模型同时覆盖陆海空等军兵种、联合保障条线，以及有人、无人和有人/无人协同平台。

非目标：模型在线推理、目标选择、武器运用、自主行动控制、未经授权采集、避开安全审查的内容处理。面向战时的内容仅限授权的保障记录、状态归档、维修医疗和复盘材料，任何行动建议都必须经过人工授权。
