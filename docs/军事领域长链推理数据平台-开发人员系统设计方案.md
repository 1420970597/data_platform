...
本系统服务于**军事领域**，但只处理经授权、非敏感、非作战化知识资料。系统的工程设计故意不包含实时态势、目标识别、目标选择、部署、战术、武器、指挥控制或行动规划能力。
...
`military_scope` 枚举建议：`authorized_non_sensitive_knowledge`、`restricted_pending_review`、`prohibited_operational`。禁止类别只能进入隔离表和审计，不能进入 `dataset_version`。
...

{
  ...
  "allowed_use": "training_non_operational_knowledge",
  "military_scope": "authorized_non_sensitive_knowledge",
...

{
  ...
  "reward_spec_id": null,
  "purpose": "training_non_operational_knowledge"
}
```
